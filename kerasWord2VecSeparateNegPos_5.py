...
class Word2VecKeras(gensim.models.word2vec.Word2Vec):
    ...
    def train(self, sentences, total_words=None, word_count=0,
        ...
        if self.sg:
            ...
            gen=train_batch_sg_our(self, sentences, sub_batch_size=sub_batch_size,batch_size=batch_size)
            self.kerasmodel.get_layer(name='embedding').set_weights([self.syn0])
            self.kerasmodel.fit_generator(gen,samples_per_epoch=35200, nb_epoch=self.iter, verbose=1)
